...
fn main() {
    ...
    match matches.value_of("centroid_count") {
        ...
        Some(val) => {
            match val.parse::<usize>() {
                Ok(parsed) => k = parsed,
                Err(_) => {
                    eprintln!("Provided k value must be a positive integer. Type 'soup -h' for help and usage.");
                    process::exit(1);
                }
            }
        }
    ...
    let image: RgbaImage;
    match image::open(matches.value_of("image_path").unwrap()) {
        Ok(img) => image = img.to_rgba(),
        Err(_) => {
            eprintln!("Could not open image at <image_path>. Type 'soup -h' for help and usage.");
            process::exit(1)
        }
    }
...
fn kmean(k: usize, is_mean: bool, image: RgbaImage) -> Vec<ColorContainer> {
    ...
    for iteration in 1..=max_iterations {
        ...
        if change && iteration == max_iterations {
            eprintln!("Reached max iteration limit of {} for kmeans.", max_iterations);
            process::exit(1);
        }
